...
const BouncingDVDLogo = () => {
  ...
  useEffect(() => {
    const changeColor = () => {
      ...
      const nextChange = Math.random() * 3000 + 1000; 
      setTimeout(changeColor, nextChange);
    };

    const initialDelay = Math.random() * 3000 + 1000;
    const timerId = setTimeout(changeColor, initialDelay);
  ...
  return (
    <div ref={containerRef} className="absolute w-full h-screen overflow-hidden">
      <div
        ref={logoRef}
        className="absolute w-24 h-12 transition-colors duration-300"
        style={{
          transform: `translate(${position.x}px, ${position.y}px)`,
        }}
      >
        <svg viewBox="0 0 210 107" fill={color}>
